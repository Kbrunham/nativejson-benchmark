...
if [ $uname = "Darwin" ]; then
    ...
    grep -m 1 "model name" /proc/cpuinfo | sed 's/model name.*: //' | sed -e 's/Intel(R)//' -e 's/(TM)//' -e 's/(R)//' -e 's/CPU//' -e 's/@/ /'| sed -E 's/^[[:space:]]+//g' | sed -E 's/[[:space:]]+/_/g' | sed 's/.*/#define MACHINE "&"/' > ../src/machine.h
fi
cat ../src/machine.h
